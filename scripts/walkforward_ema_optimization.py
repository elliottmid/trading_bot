...
# Date: 2026-05-14
# Description: Annual walk-forward EMA optimization — each year finds Sharpe-maximizing
#              entry/exit EMA + trailing stop on prior 9-year IS window (108-month optimum),
#              then applies
#              those params OOS to the following calendar year (2010–2026).
#
# Grid: entry EMA (fast 3-12, slow 8-26), exit EMA (same), trailing stop 3-6%.
...
IS_WINDOW   = 9                          # 108-month IS window — lookback sweep optimum (vs 120m default)
TC_BPS      = 5
...
# ── fast IS backtest — returns Sharpe only (O(1) space via running accumulation) ─
...
def _sharpe_only(
    ...
    for i in range(2, n):
        ...
        if in_pos:
            ...
            daily_ret    += eq - prev_eq       # incremental MTM (zero on entry bar)
            prev_eq      = eq
...
def _run_is_batch(
    ...
    for ef, es, xf, xs, ts in combo_batch:
        sharpe = _sharpe_only(
            close_arr,
            ema_dict[ef], ema_dict[es], ema_dict[xf], ema_dict[xs],
            ts, tc_frac,
        )
...
def backtest_full(
    ...
    n         = len(close_arr)
    tc_frac_  = tc_frac  # already passed
    ...
    return {
        "trades":       n_trades,
        "oos_strat_ret": strat_ret,   # raw holding-period return (for chain-linking)
        "cagr":         cagr,
        "sharpe":       sharpe,
        "max_dd":       max_dd,
        "win_rate":     win_rate,
...
def run_walkforward(df: pd.DataFrame, n_jobs: int = -1) -> pd.DataFrame:
    ...
    # All unique EMA periods needed across the entire grid
    ...
    for oos_year in WF_YEARS:
        is_start = oos_year - IS_WINDOW   # e.g. 2000 for oos_year=2010
        is_end   = oos_year - 1
        ...
        for symbol in SYMBOLS:
            ...
            # ── IS slice: precompute EMAs, build numpy arrays ──────────────
            is_mask = (
            ...
            logger.info(
                ...
                f"entry EMA({ef}/{es})  exit EMA({xf}/{xs})  trail={ts:.0%}"
            )

            # ── OOS: compute EMAs on combined [is_start → oos_year] window ─
            # Critical: EMAs must be initialised on pre-OOS data to avoid
            # look-ahead and ensure correct EMA initial values for OOS bars.
            combined_mask = (
                (sym_all["timestamp"].dt.year >= is_start) &
...
def compile_summary(results_df: pd.DataFrame) -> None:
    print("\n" + "=" * 130)
    print("WALK-FORWARD EMA OPTIMIZATION — OOS RESULTS  "
    ...
    for symbol in SYMBOLS:
        ...
        # Chain-link using raw holding-period returns (oos_strat_ret), not CAGRs
...
def plot_results(results_df: pd.DataFrame, today_str: str) -> None:
    ...
    out_path = RESULT_DIR / f"walkforward_ema_equity_curve_{today_str}.png"
    ...
    fig.suptitle(
        "Walk-Forward EMA Optimization — Annual IS/OOS Results (2010–2026)\n"
        f"IS window: 10 years | Grid: EMA entry/exit × trailing stop 3–6% | TC: {TC_BPS} bps | IS objective: Sharpe",
        fontsize=12,
    )

    # ── panel 1: annual OOS CAGR bar chart ────────────────────────────────
    ax1 = axes[0]
    for i, sym in enumerate(SYMBOLS):
        ...
        # Annotate bars with the year's selected trailing stop
    ...
    # ── panel 2: chain-linked equity curves ───────────────────────────────
    ax2 = axes[1]
    for sym in SYMBOLS:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Annual walk-forward EMA optimization (Sharpe-maximizing IS → OOS)"
    )
    parser.add_argument(
    ...
    RESULT_DIR.mkdir(parents=True, exist_ok=True)
    csv_path = RESULT_DIR / f"walkforward_ema_results_{today_str}.csv"
